...
# Check for sufficient data before processing
n_runs <- length(unique(data$Run))
lvls <- levels(as.factor(data$Condition))
l <- length(lvls)

if (n_runs < 2 || l < 2) {
    print(paste0("Insufficient data for statistical analysis: ", n_runs, " run(s), ", l, " condition(s). ",
                 "MSstats requires at least 2 runs and 2 conditions. Skipping."))
    # Write empty results so downstream processes have expected outputs
    write.csv(data.frame(), file=paste0(args[8], "_comparisons.csv"), row.names=FALSE)
} else {
