...
def generate_tool_config(
    ...
    # New tool-specific generators
        ...
        # generate_speclibgen_config returns a dict; convert to key=value for file output
        config_dict = generate_speclibgen_config(params)
        content = "\n".join(f"{k} = {v}" for k, v in config_dict.items())
        filename = (
            "speclibgen.params"  # Key=value properties format (for reference only)
        )
